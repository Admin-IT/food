...
( 
       menu_user_id varchar(50) primary key,-- 唯一ID   格式:yh100001
...
(
   menu_user_folow_id varchar(50) not null, -- 关注用户ID
...
(
    menu_user_id varchar(50) not null, -- 收藏用户ID
...
(
    menu_user_id varchar(50) not null, -- 用户ID
		menu_article_id varchar(50) not null -- 文章ID
);
...
(
     menu_user_id varchar(50) not null, -- 用户ID
		 ...
		 menu_sender_id varchar(50), -- 发送者ID 
...
(
    menu_food_id varchar(50) primary key, -- 菜谱ID 格式：f1000001 唯一
...
(
   menu_ingredients_id varchar(50) primary key, -- 唯一ID 格式: is1000001
...
(
   menu_article_id varchar(50) primary key, -- 唯一ID 格式：tx100001
...
(
   menu_comment_id varchar(50) primary key, -- 评论ID
	 menu_comment_content varchar(200) null , -- 评论内容
	 menu_user_id varchar(50) null -- 用户ID 
);
...
(
  ...
  menu_reply_id int primary key , -- 回复ID
	menu_reply_text varchar(200) not null , -- 回复内容
	menu_reply_user_id varchar(50) not null , -- 用户ID
...
(
   menu_article_id varchar(50) not null -- 展示内容
);

-- 内容展示(热门)
create table menu_floor_four
(
   menu_content_label varchar(50) not null -- 展示内容
);
...
(
   menu_video_id varchar(50) primary key , -- 视频ID 格式： vd100001
	 ...
	 menu_video_admin varchar(50) not null , -- 上传者
...
(
   menu_push_admin varchar(50) not null , -- 推送人 管理员
	 menu_push_content varchar(500) not null , -- 推送内容
	 menu_push_user varchar(50) not null , -- 接收人 用户
...
create table menu_examine_article
 (
    menu_article_id varchar(50) not null , -- 文章ID
		...
		menu_examine_admin varchar(50) not null , -- 审核人 管理员
